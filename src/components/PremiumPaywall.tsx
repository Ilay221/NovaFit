...
import { Crown, Check, Shield, Zap, ArrowRight, Loader2 } from 'lucide-react';
...
const features = [
  { icon: Zap, label: 'AI-powered food scanning' },
  { icon: Shield, label: 'Advanced analytics & insights' },
  { icon: Check, label: 'Unlimited meal history' },
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-5">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center mb-8">
          <h1 className="text-[28px] font-bold font-display tracking-tight mb-2">Unlock Premium</h1>
          <p className="text-sm text-muted-foreground leading-relaxed">
            Get the most out of NovaFit with lifetime access to all premium features.
          </p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="text-center mb-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">Lifetime access. No recurring charges.</p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="space-y-3">
          <Button
            ...
            disabled={loading}
            className="w-full h-13 rounded-2xl text-[15px] font-semibold active:scale-[0.98] transition-transform"
          >
            {loading ? (
        ...
        <motion.div variants={itemVariants} className="flex items-center justify-center gap-2 mt-6 text-[11px] text-muted-foreground">
          ...
          <span>Secured by Stripe. Cancel-free purchase.</span>
        </motion.div>
      </motion.div>
    </div>
  );
